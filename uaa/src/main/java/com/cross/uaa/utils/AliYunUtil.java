...
public class AliYunUtil {
    ...
    /**
     * 主队列   //QUEUE_NAME_MASTER
     */
    private String _queueNameMaster;
    /**
     * 备份队列  //QUEUE_NAME_BAK
     */
    private String _queueNameBak;
    /**
     * 失败队列  //QUEUE_NAME_FAIL
     */
    private String _queueNameFail;
     ...
     * @return
     */
    public Message popSendOrderMasterQueue() {
        //log.debug("pop queue start: ");
        Message message = this.popQueue(_sendOrderQueueName);
        //log.trace("pop queue end: ", message);
        return message;
    }
    ...
    public void delayRequestTime(Message message) {
        ...
        try{
            ...
            if (message.getDequeueCount() > failTryCount) {
                ...
                log.error("tried push data count too large. {}", message);
            }
            else{
     ...
     * @return
     */
    public Message delSendOrderQueue(Message message) {
        CloudQueue queue = client.getQueueRef(_sendOrderQueueName);
        log.info(String.format("delete queue by: %s", _sendOrderQueueName));
        queue.deleteMessage(message.getReceiptHandle());
        return message;
    }
