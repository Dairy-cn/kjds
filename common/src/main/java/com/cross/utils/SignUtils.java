...
public class SignUtils {
     ...
     * @return
     */
    public static String getSign(Map<String, Object> requestMap, String appSecret) {
        //请求参数键值升序排序
        ...
        for(String key : list){
            if (requestMap.get(key) == null || requestMap.get(key).equals("") || key.equalsIgnoreCase("sign")) {
                continue;
            }
            signStr.append(key).append("=");
            signStr.append(requestMap.get(key));
            signStr.append("&");
        }
        //MD5签名并校验
        String sign = MD5.sign(signStr.toString(),"key=" + appSecret, "utf-8");
